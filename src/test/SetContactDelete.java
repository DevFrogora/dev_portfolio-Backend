...
@WebServlet("/SetContactDelete") // Annotation
public class SetContactDelete extends HttpServlet {

	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		// TODO Auto-generated method stub
		resp.addHeader("Access-Control-Allow-Origin", "*");
		ObjectMapper objectMapper = new ObjectMapper();
		ContactDelete contactDelete = objectMapper.readValue(req.getInputStream(), ContactDelete.class);
		Date date = new Date();
...
//			System.out.println(messagesJson);
			...
			List<Message> messages = objectMapper.readValue(messagesJson, new TypeReference<ArrayList<Message>>() {
			});
			
			for (Iterator iterator = messages.iterator(); iterator.hasNext();) {

				Message message = (Message) iterator.next();
				if (message.getDate().equals(contactDelete.getDate())) {
